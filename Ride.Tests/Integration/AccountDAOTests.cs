...
public sealed class AccountDAOTests : IDisposable
{
    ...
    [Fact(DisplayName = "Should create an account in database and get by email")]
    public async void Create_WhenDataAreValid_ShouldCreateAccountInDatabase()
    {
        var account = Account.Create("John Doe", $"john.doe{Guid.NewGuid().ToString()}@gmail.com", "95818705552", true,
            false, "");
